...
sys.path.insert(0, '/home/jovyan/srinjoy-vol/Tasmiah_files/Motion-Transfer-Keypoints-Prediction/Keypoints_Prediction/stochastic_normalizing_flows/bgtorch/bgtorch/nn/flow/stochastic')

from mcmc import MetropolisMCFlow
from langevin import LangevinFlow

sys.path.insert(0, '/home/jovyan/srinjoy-vol/Tasmiah_files/Motion-Transfer-Keypoints-Prediction/Keypoints_Prediction/stochastic_normalizing_flows/snf_code/snf_code')

from snf import InterpolatedEnergy
...
class BatchNorm(nn.Module):
    ...
    def forward(self, x, cond_y=None):
        ...
        # Safely match the shape of `x`
        while log_abs_det_jacobian.ndim < x.ndim:
            log_abs_det_jacobian = log_abs_det_jacobian.unsqueeze(0)
        
        log_abs_det_jacobian = log_abs_det_jacobian.expand_as(x)
...
class SRealNVP(nn.Module):
    def __init__(
        ...
        for _ in range(n_blocks):
            ...
            mask = 1 - mask  # flip mask for next block

        self.net = FlowSequential(*modules)

        # Add MCMC layers (one after each NF block)
        self.mcmc_layers = nn.ModuleList([
            MetropolisMCFlow(nsteps=mcmc_steps, stepsize=mcmc_eps)
            for _ in range(n_blocks)
        ])

        # Register base distribution
        self.register_buffer("base_dist_mean", torch.zeros(input_size))
        self.register_buffer("base_dist_var", torch.ones(input_size))
    ...
    def inverse_with_mcmc(self, u, cond, energy_model, context):
        sum_log_abs_det_jacobians = None
        mcmc_counter = 0
        module_list = list(reversed(self.net))
        T = len(self.mcmc_layers)  # Usually == n_blocks
        i = 0
        while i < len(module_list):
            module = module_list[i]
            #print("Module:", module.__class__.__name__)
            u, log_abs_det_jacobian = module.inverse(u, cond)
            #print(f"u shape: {u.shape}")
            if sum_log_abs_det_jacobians is None:
                sum_log_abs_det_jacobians = torch.zeros_like(log_abs_det_jacobian)
            # print("sum_log_abs_det_jacobians:", sum_log_abs_det_jacobians.shape)
            # print("log_abs_det_jacobian:", log_abs_det_jacobian.shape)
            sum_log_abs_det_jacobians += log_abs_det_jacobian
            i += 1

            # ✅ Apply MCMC only after RealNVP (LinearMaskedCoupling) blocks
            if isinstance(module, LinearMaskedCoupling) and mcmc_counter < T:
                lambda_ = mcmc_counter * 1.0 / (T - 1) if T > 1 else 1.0
                def prior_energy(y):
                    return 0.5 * torch.sum(y ** 2, dim=-1, keepdim=True)
    
                def target_energy(y):
                    return energy_model(y, context)

                interpolated_energy_fn = InterpolatedEnergy(
                    prior_energy,target_energy,lambda_
                )

                # initialize velocity to zeros

                u, _ = self.mcmc_layers[mcmc_counter]._forward(u, energy_model = interpolated_energy_fn)
                mcmc_counter += 1
    
        return u, sum_log_abs_det_jacobians

    def sample_with_energy(self, cond, energy_model, context):
        """
        Sample with SNF: base → RealNVP → MCMC → ...
        cond: GRU conditioning vector
        context: past sequence for GRU energy
        """
        shape = cond.shape[:-1]  # [batch]
        u = self.base_dist.sample(shape)  # z ~ N(0, I)
        x, _ = self.inverse_with_mcmc(u, cond, energy_model, context)
        return x
